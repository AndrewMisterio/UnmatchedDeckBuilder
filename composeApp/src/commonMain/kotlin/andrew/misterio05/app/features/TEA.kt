...
interface ReducerResult<S: State> {
    ...
    companion object {
        ...
        data class ReducerResultImpl<S: State>(override val state: S, override val effects: Set<Effect>):
            ReducerResult<S>

        operator fun <S: State, E: Effect> invoke(state: S, vararg effects: E?) = ReducerResultImpl(state, setOfNotNull(*effects))
        operator fun <S: State, E: Effect> invoke(state: S, effects: Set<E?>) = ReducerResultImpl(state, effects.filterNotNull().toSet())
    }
}
